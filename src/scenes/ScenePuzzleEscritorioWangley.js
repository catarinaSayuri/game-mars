export default class ScenePuzzleEscritorioWangley extends Phaser.Scene {
    constructor() {
        super({ key: "ScenePuzzleEscritorioWangley" });
    }

    preload() {
        this.carregarAssetsPersonagens();
        this.carregarAssetsInterface();
        this.load.on('complete', () => console.log('Assets do minigame Wangley carregados'));
        this.load.on('loaderror', (file) => console.error('Erro ao carregar:', file.key));
    }

    carregarAssetsPersonagens() {
        const personagens = [
            "popUpLucasRocha", "popUpMatheusReis", "popUpNataliaBarros",
            "popUpPedroNunes", "popUpSabrinaMelo", "popUpThiagoMendes"
        ];
    ...
    create(data) {
        this.personagensPorSetor = {
            wangley: [
                { nome: "Lucas Rocha", soft: 3, hard: 3, imagem: "popUpLucasRocha" },
                { nome: "Matheus Reis", soft: 2, hard: 4, imagem: "popUpMatheusReis" },
                { nome: "Natalia Barros", soft: 4, hard: 2, imagem: "popUpNataliaBarros" },
                { nome: "Pedro Nunes", soft: 1, hard: 5, imagem: "popUpPedroNunes" },
                { nome: "Sabrina Melo", soft: 5, hard: 1, imagem: "popUpSabrinaMelo" },
                { nome: "Thiago Mendes", soft: 3, hard: 3, imagem: "popUpThiagoMendes" }
            ]
        };

        this.setorAtual = "wangley";
